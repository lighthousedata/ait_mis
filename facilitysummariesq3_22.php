  ...
  <div class="header">
	...
	?>
         ...
         <div class="col-lg-12">
				<?php
				// 1querying date
				$selectchileka=mysql_query("SELECT * FROM indexreg where  dateidentified > '2022-08-31' and dateidentified < '2022-12-01' ");
				$totindexreg=mysql_num_rows($selectchileka);
				
				$identfiedck=mysql_query("SELECT * FROM indexreg WHERE  dateidentified > '2022-08-31' and dateidentified < '2022-12-01' ");
				$offeredck=mysql_num_rows($identfiedck);
				
				$resultck= mysql_query("SELECT sum(elicitedcontacts) AS Contacts  FROM indexdetails Where dateidentified > '2022-08-31' and dateidentified < '2022-12-01' ");
				$rowck = mysql_fetch_assoc($resultck); 
				$sumck = $rowck['Contacts'];
				
				$acceptck=(mysql_query("SELECT * FROM indexreg WHERE facility like '%Umodzi%' AND consent = 'Yes' and dateidentified > '2022-08-31' and dateidentified < '2022-12-01'")); 
				$totaccept=mysql_num_rows($acceptck);
				
				$eligibleck= mysql_query("SELECT sum(eligibles) AS Eligible  FROM indexdetails Where dateidentified > '2022-08-31' and dateidentified < '2022-12-01' ");
				$ckeligible = mysql_fetch_assoc($eligibleck); 
				$toteligible = $ckeligible['Eligible'];
				
				$testvpchileka=mysql_query("SELECT * FROM reachedContacts where outcomedate > '2022-08-31'  AND outcomedate <'2022-12-01' AND outcome <>'Not Tested' AND outcome
				...
				$reachedchileka=mysql_query("SELECT * FROM reachedContacts where outcomedate > '2022-08-31'  AND outcomedate <'2022-12-01' ");
				$reachedck=mysql_num_rows($reachedchileka);
				
				$reacheeligiblechileka=mysql_query("SELECT * FROM reachedContacts where outcomedate > '2022-08-31'  AND outcomedate <'2022-12-01' AND outcome <> 'Previous Positive on ART' ");
				$reachegck=mysql_num_rows($reacheeligiblechileka);
				
				$contactsposck=mysql_query("SELECT * FROM reachedContacts where outcomedate > '2022-08-31'  AND outcomedate <'2022-12-01' AND outcome = 'New positive' ");
