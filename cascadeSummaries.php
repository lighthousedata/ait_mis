...
<table class="table table-bordered" width="100%"  border="0" style="padding-left:40px">
	...
	?>
         ...
         <div class="col-lg-12">
				...
				$selectTisungane=mysql_query("SELECT * FROM indexreg where facility='Tisungane' AND dateidentified between '$fdate' and '$tdate' ");
				$totindexreg=mysql_num_rows($selectTisungane);
				
				$identfiedck=mysql_query("SELECT * FROM indexreg WHERE facility = 'Tisungane' AND dateidentified between '$fdate' and '$tdate' ");
				$offeredck=mysql_num_rows($identfiedck);
				
				$resultck= mysql_query("SELECT sum(elicitedcontacts) AS Contacts  FROM indexdetails Where facility = 'Tisungane'AND dateidentified between '$fdate' and '$tdate' ");
				$rowck = mysql_fetch_assoc($resultck); 
				$sumck = $rowck['Contacts'];
				
				$acceptck=(mysql_query("SELECT * FROM indexreg WHERE facility = 'Tisungane' AND consent = 'Yes' AND dateidentified between '$fdate' and '$tdate' ")); 
				$totaccept=mysql_num_rows($acceptck);
				
				$eligibleck= mysql_query("SELECT sum(eligibles) AS Eligible  FROM indexdetails Where facility = 'Tisungane' AND dateidentified between '$fdate' and '$tdate' ");
				$ckeligible = mysql_fetch_assoc($eligibleck); 
				$toteligible = $ckeligible['Eligible'];
				
				$testvpTisungane=mysql_query("SELECT * FROM reachedcontacts where facility='Tisungane' AND outcome= 'New Negative' AND outcomedate between '$fdate' and '$tdate' ");
				$testedvp=mysql_num_rows($testvpTisungane);
												
				$reachedTisungane=mysql_query("SELECT * FROM reachedcontacts where facility='Tisungane' AND outcomedate between '$fdate' and '$tdate' ");
				$reachedck=mysql_num_rows($reachedTisungane);
				
				$reacheeligibleTisungane=mysql_query("SELECT * FROM reachedcontacts where facility ='Tisungane' AND outcome <> 'Previous Positive on ART' AND outcomedate between '$fdate' and '$tdate' ");
				$reachegck=mysql_num_rows($reacheeligibleTisungane);
				
				$contactsposck=mysql_query("SELECT * FROM reachedcontacts where facility='Tisungane' AND outcome = 'New positive' AND outcomedate between '$fdate' and '$tdate' ");
...
<?php		
//1 Tisungane					
 echo "<tr bgcolor='#FFFFFF'><td><font size= '+1'>&nbsp;Tisungane</font></td><td><font size= '+1'>&nbsp;&nbsp;&nbsp;$totindexreg</font>
