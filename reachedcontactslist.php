...
<center> <body>
  	...
  	<h2>List of reached contacts </h2>
	<hr >
...
<?php
$selectallpatients=mysqli_query($db,"SELECT * FROM reachedcontacts WHERE outcomedate between '$fdate' and '$tdate'  order by outcomedate");
...
$selectallpatients=mysqli_query($db,"SELECT * FROM reachedcontacts WHERE facility LIKE '%$reportvalue%' AND outcomedate between '$fdate' and '$tdate'  order by outcomedate");
$numrows=mysqli_num_rows($selectallpatients);
 }else if($reportterm=='gender'){
$selectallpatients=mysqli_query($db,"SELECT * FROM reachedcontacts WHERE gender LIKE '%$reportvalue%' AND outcomedate between '$fdate' and '$tdate'  order by outcomedate");
$numrows=mysqli_num_rows($selectallpatients);
 }else if($reportterm=='outcome'){
$selectallpatients=mysqli_query($db,"SELECT * FROM reachedcontacts WHERE outcome LIKE '%$reportvalue%'AND outcomedate between '$fdate' and '$tdate'  order by outcomedate");
$numrows=mysqli_num_rows($selectallpatients);

}else if($reportterm=='contnum'){
$selectallpatients=mysqli_query($db,"SELECT * FROM reachedcontacts WHERE contnum LIKE '%$reportvalue%' ");
$numrows=mysqli_num_rows($selectallpatients);
...
<tr bgcolor="#666666"><td><font size= "+1"><b>Index vist date</font></td><td><font size= "+1"><b>Facility Name</font></td><td><font size= "+1"><b>Contact Number</font>
 ...
 <font size= '+1'>$age</font></td><td><font size= '+1'>$ipvstatus</font></td><td><font size= '+1'>$outcome</font></td><td><font size= '+1'>$outcomedate</font></td><td>&nbsp;&nbsp;<a href='edit_contactsoutcomes.php?editclientid=$uniqueid'>Edit</font></td></tr> ";
		} 
	} 
}
}?>
